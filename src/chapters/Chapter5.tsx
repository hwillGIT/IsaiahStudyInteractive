...
import { Chapter5StructureModal } from '../components/Chapter5StructureModal';
...
// Color-code transformation points by type ('center' same as 'hinge')
const getHingeColorOverride = (hingeType?: string): string => {
  const colors: Record<string, string> = {
    'center': 'bg-blue-500',
    'climax-woes': 'bg-purple-500'
  };
...
function Chapter5() {
  ...
  const { data, loading, error } = useChapterData(5);

  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-xl text-gray-600">Loading Chapter 5...</div>
      </div>
    );
  }
    ...
    <div className="min-h-screen p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold mb-2 text-gray-800">Isaiah Chapter 5</h1>
          <p className="text-xl text-gray-600">The Song of the Vineyard and Six Woes</p>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 mb-8">
          {verses.map((verse) => (
            ...
            >
              <div className="text-sm font-bold">5:{verse.number}</div>
              {verse.isHinge && (
              ...
              <div className="hidden group-hover:block absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 w-64 bg-gray-900 text-white text-xs rounded py-2 px-3 z-10">
                <div className="font-bold mb-1">Verse 5:{verse.number}</div>
            ...
            <div className="bg-white rounded-lg max-w-2xl w-full my-8">
              <div className="p-6 border-b border-gray-200 bg-gradient-to-r from-purple-600 to-blue-600 text-white rounded-t-lg">
                <div className="flex justify-between items-start mb-4">
                  <h3 className="text-2xl font-bold">Isaiah 5:{selectedVerse}</h3>
                  <button
